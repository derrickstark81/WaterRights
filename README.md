...
flowchart LR
  ...
  direction TB
    ...
    BATCH["Batch Server
app-bat01-835.agency.ok.local
E:\Updates\ArcUpdateScripts"]

    NOTE(("Task Scheduler
runs wr_updates.py"))

    OWRP["OWRP Oracle (Cheyenne)
dat-orc01-835 / owrp.owrb.thinkbig"]

    OWRPGIS["OWRPGIS
Oracle SDE 11gR2 (PROD)"]

    OWRTGIS["OWRTGIS
Oracle SDE 11gR2 (TEST)"]

    ARAPAHO["Arapaho server
\\Arapaho\MasterCovs\z_GDBs\Water_Rights.gdb"]

    SCISSOR["Scissortail server
\\Scissortail\MasterCovs"]

    OWRBGIS["OWRBGIS server
\\OWRBGIS\GeoDat"]

    WEBFINAL["WebFinal share
\\172.30.73.39\WebFinal"]
  ...
  direction TB
    CSA_PG["CSA PostgreSQL
owrp_csa.sde"]
    CSA_PORTAL["CSA ArcGIS Enterprise
Portal / Server / Data Store"]
  end
  ...
  direction TB
    ODP["OWRB Open Data (future)
home-owrb.opendata.arcgis.com"]
    PUB_USERS["Public and Partners"]
  end
  class Public_Access pub

  %% ---------- FLOWS ----------
  BATCH -->|Launch nightly| NOTE
  NOTE -->|Reads config: settings.json| BATCH
